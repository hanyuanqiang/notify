...
<script>
    ...
    function showDetail(id) {
        ...
        /* 把用户名和密码通过post方式传到后台 */
        xmlHttp.open("post", "/notify_Web/yzfw/llhd", true);
...
<div hidden="hidden" id="publish" style="border-style: solid;border-width: 2px;border-color: #1b6d85">
    <div style="margin-left: 20px">
        <h3 align="center">发布邻里互动信息</h3>
        <form action="/notify_Web/yzfw/llhd?action=publish&mark=<%=request.getParameter("mark")%>" method="post" onsubmit="return submitCheck()">
...
<div id="showPage">
    ...
    <div style="margin-bottom: 10px">
        <form action="/notify_Web/yzfw/llhd?mark=shequ" method="post" id="s_form">
        <c:if test="${unique=='shequ'}">
            <script>
                $("#s_form").attr("action","/notify_Web/yzfw/llhd?mark=shequ");
            </script>
        ...
        <c:if test="${unique=='wuye'}">
            <script>
                $("#s_form").attr("action","/notify_Web/yzfw/llhd?mark=wuye");
            </script>
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-control" name="contentType" style="width: 150px">
                            ...
                            <option value="小区拼车" ${contentType=="小区拼车"?'selected':'' }>小区拼车</option>
                            <option value="租赁求租" ${contentType=="租赁求租"?'selected':'' }>租赁求租</option>
                            <option value="育儿育婴" ${contentType=="育儿育婴"?'selected':'' }>育儿育婴</option>
                            <option value="失物招领" ${contentType=="失物招领"?'selected':'' }>失物招领</option>
                            <option value="二手交易" ${contentType=="二手交易"?'selected':'' }>二手交易</option>
    ...
    <p align="center">数据列表<button style="float: right;margin-right: 30px;" class="btn btn-success btn-xs" onclick="onClickPublishButton()">发&nbsp;布</button></p>
    <div style="border-top:1px dashed #cccccc;height: 1px;overflow:hidden;margin: 5px 20px;"></div>
    <div>
        <ul>
            <c:forEach items="${llhdDatas}" var="data">
                <li><a href="#" onclick="showDetail(${data.id})" data-toggle="modal"
                    ...
                    <c:if test="${unique=='shequ'}">
                        <a class="btn btn-danger btn-xs" href="/notify_Web/yzfw/llhd?action=delete&mark=shequ&id=${data.id}" onclick="return confirm('确定删除这条记录？')" style="margin-left: 50px;">
                            删除
                        </a>
                    </c:if>

                    <c:if test="${unique=='wuye'}">
                        <a class="btn btn-danger btn-xs" href="/notify_Web/yzfw/llhd?action=delete&mark=wuye&id=${data.id}" onclick="return confirm('确定删除这条记录？')" style="margin-left: 50px;">
                            删除
                        </a>
                    </c:if>
